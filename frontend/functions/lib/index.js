...
const getMonzoConfig = () => {
    ...
    const redirectUri = cfg.monzo?.redirect_uri ?? process.env.MONZO_REDIRECT_URI ?? 'https://tasha.heysalad.app/api/monzo/callback';
...
const getElevenLabsConfig = () => {
    ...
    const defaultVoiceId = cfg.elevenlabs?.voice_id ?? process.env.ELEVENLABS_VOICE_ID ?? 'pNInz6obpgDQGcFmaJgB';
    if (!apiKey || !agentId) {
        throw new Error('11Labs API key or agent ID not configured');
    }
    return { apiKey, agentId, defaultVoiceId };
};
const isFoodRelated = (transaction) => {
...
const analyzeFoodSpending = (transactions) => {
...
app.post('/voice/audio', async (req, res) => {
    try {
        ...
        const response = await fetch(`https://api.elevenlabs.io/v1/conversational-ai/conversations/${conversationId}/audio`, {
            method: 'POST',
            headers: {
                'xi-api-key': apiKey
            },
...
app.post('/monzo/match', async (req, res) => {
    try {
        ...
        const data = (await response.json());
        const foodTransactions = (data.transactions || []).filter(isFoodRelated);
        ...
        const calculateMatchScore = (transaction) => {
            let score = 0;
            const wasteKeywords = extractKeywords(wasteDescription);
            const merchantName = transaction.merchant?.name?.toLowerCase() || '';
            const description = transaction.description.toLowerCase();
            const matchingKeywords = wasteKeywords.filter((keyword) => merchantName.includes(keyword) || description.includes(keyword));
